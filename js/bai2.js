 ...
 *
 * Billing mirrors Translation and Data Extraction: per page, charged as
 * each page completes, through window.LexoraBilling.
 */
(function () {
  'use strict';
  ...
  function fileRows() {
    ...
    return STATE.files.map(function (f) {
      ...
      const action = f.status === 'Success'
        ? `<a class="file-action-link" onclick="Bai2.downloadOne(${f.uid})">Download</a>`
        : (f.status === 'Failed'
      ...
      return `<tr>
        ...
        <td><div class="progress-bar-container">
          ...
          <span class="progress-label">${pct}%</span></div></td>
        <td>${action}</td>
      </tr>`;
    }).join('');
  }
