...
DOCUMENT_RESOURCES = (
    ...
    "ai-prompts",
    # Translation Rules - a growing, admin-editable list of corrections
    # learned from real translation review feedback (terminology
    # preferences, phrasing fixes, things NOT to shorten/simplify). Read
    # by the browser-side translation pipeline (js/engine-translation.js)
    # at translate-time and appended into that run's prompt, so a rule
    # added here takes effect on the very next translation - no code
    # change or redeploy needed. This is the actual "self-improving"
    # mechanism: every reviewer correction becomes one more row here
    # instead of a one-off prompt edit, and it's editable straight from
    # the PostgreSQL admin table like any other resource.
    "translation-rules",
    # Translation Domains - the SELF-EXPANDING counterpart to the 13
    # hardcoded domain-expert personas in js/engine-translation.js
    # (TRANSLATION_DOMAIN_EXPERTS - Real Estate, Legal, Healthcare, ...).
    # When a document's classified domain doesn't match any of those 13
    # (or any domain already saved here from a PREVIOUS document), the
    # pipeline generates a new {role, terms} expert profile for it via
    # its own LLM call and saves it here automatically - so the very
    # next document in that same domain (e.g. "Aviation", "Maritime
    # Law") reuses the saved profile instantly instead of generating one
    # again, and gets the exact same translator+reviewer persona
    # treatment the original 13 domains get. Admin-editable like any
    # other resource, so a generated profile can be refined by hand too.
    "translation-domains",
    # Translation Code Issues - the reviewer pass (v14ReviewTranslation,
    # js/engine-translation.js) flags a SEPARATE category here whenever it
    # spots something that looks like a systematic/structural pipeline
    # bug (e.g. the same trailing phrase duplicated after several
    # unrelated sentences, or every sub-clause showing the same letter) -
    # as opposed to an ordinary one-off wording correction, which it
    # applies directly and doesn't need a human to look at. This is
    # DETECTION ONLY: the reviewer flags a row here and moves on - it
    # never edits code itself, and nothing here gets acted on
...
VIRTUAL_TABLES = {
    ...
    "doc_translation_rules": {
        "kind": "document",
        "resource": "translation-rules",
        # active=No lets an Admin switch a rule off (e.g. it turned out
        # wrong, or only applied to one document) without deleting the
        # row and losing the history of why it was added.
        "fields": ["id", "ruleText", "category", "active", "note"],
        "types": {},
        "jsonb_defaults": {"active": "Yes"},
        "field_defaults": {"active": "Yes"},
        "select_options": {"active": ["Yes", "No"],
                            "category": ["Terminology", "Formatting", "Quality Check", "Other"]},
        "readonly_fields": set(),
    },
    "doc_translation_domains": {
        "kind": "document",
        "resource": "translation-domains",
        # domain: the classifier-detected domain name (e.g. "Aviation") -
        # role/terms: the same shape as the 13 hardcoded personas in
        # TRANSLATION_DOMAIN_EXPERTS (js/engine-translation.js), just
        # generated on first encounter instead of hand-written.
        # autoGenerated=Yes marks a profile the pipeline created itself
        # (vs one an Admin wrote/edited by hand) - purely informational,
        # doesn't change how it's used.
        "fields": ["id", "domain", "role", "terms", "autoGenerated"],
        "types": {},
        "jsonb_defaults": {"autoGenerated": "Yes"},
        "field_defaults": {"autoGenerated": "Yes"},
        "select_options": {"autoGenerated": ["Yes", "No"]},
        "readonly_fields": set(),
    },
    "doc_translation_code_issues": {
        "kind": "document",
        "resource": "translation-code-issues",
        # status: New (just flagged, nobody's looked at it) / Reviewed
        # (a human looked, confirmed it's real, not yet fixed) / Fixed /
        # Not a Bug (human looked, decided it's not actually a code
        # issue - e.g. it was a genuine one-off, or a translation-
        # quality call, not a pipeline defect). The reviewer pass only
        # ever writes rows with status=New - a human changes it from
        # there; the pipeline never sets Fixed/Not a Bug itself.
        "fields": ["id", "description", "evidence", "domain", "docType", "flaggedAt", "status"],
        "types": {},
...
VIRTUAL_TABLE_LABELS = {
    ...
    "doc_translation_rules": {"id": "ID", "ruleText": "Rule (goes into every translation prompt)",
                               "category": "Category", "active": "Active",
                               "note": "Note (why this rule was added, for your own reference)"},
    "doc_translation_domains": {"id": "ID", "domain": "Domain", "role": "Expert Persona (role)",
                                 "terms": "Terminology Glossary", "autoGenerated": "Auto-generated"},
    "doc_translation_code_issues": {"id": "ID", "description": "Issue Description",
