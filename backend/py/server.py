...
# server.py lives in backend/py/. ROOT_DIR (one level up from this file)
# resolves to backend/ - json/, Users/, Template/, "AI Prompts/", .env, and
# py/fonts are all still direct children of backend/, so every existing
# ROOT_DIR-relative path below is unchanged code and still correct.
#
# The static-served website (index.html, css/, js/, Pictures/) lives in a
# SEPARATE, sibling frontend/ folder instead (project/frontend/ next to
# project/backend/) - FRONTEND_DIR points the HTTP static-file handler and
# the handful of places that read a frontend asset (e.g. the company logo
# under Pictures/) at that folder instead of ROOT_DIR. See the Handler
# class and its translate_path override below for how request URLs get
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def translate_path(self, path):
        # json/ and Users/ live under backend/ (ROOT_DIR), not frontend/
        # (self.directory) - route requests for those two specifically
        # back to ROOT_DIR so they keep serving exactly as before, while
        # everything else (index.html, css/, js/, Pictures/) resolves
        # against frontend/ as usual.
        url_path = path.split("?", 1)[0].split("#", 1)[0]
        if url_path == "/json" or url_path.startswith("/json/") or url_path == "/Users" or url_path.startswith("/Users/"):
    ...
    def do_GET(self):
        ...
        get_routes = {
            "/api/terms": self._handle_terms_get,
    ...
    def _handle_terms_get(self, query):
        # Reads the file fresh on every call (no caching) so an admin
        # editing/replacing backend/json/terms-and-conditions.txt takes
        # effect immediately, with no code change or restart needed.
        path = os.path.join(JSON_DIR, "terms-and-conditions.txt")
        try:
            with open(path, "r", encoding="utf-8") as f:
                content = f.read()
        except OSError:
    ...
    def do_POST(self):
        ...
        routes = {
            "/api/terms/accept": self._handle_terms_accept,
                ...
                if user.get("email"):
                    def _send_receipt_email_worker(user=user, entry=entry):
                        try:
                            ...
                            logo_path = os.path.join(FRONTEND_DIR, logo_rel) if logo_rel else ""
    ...
    def _handle_terms_accept(self, body):
        user_id = self._authenticated_user_id()
        users = auth_store.load_users()
        user = auth_store.find_user_by_id(users, user_id)
        if not user:
        ...
        user["termsAcceptedAt"] = datetime.datetime.utcnow().isoformat() + "Z"
        auth_store.save_users(users)
        return 200, {"ok": True}
    ...
    def _handle_ocr_analyze_strategy(self, body):
        ...
        tables or background colors) or not? Returns just the decision,
        not a converted document - the caller uses this to decide which
        engine to actually invoke:
          ...
          - "lightweight" -> call the EXISTING client-side
                             ...
                             captures real embedded images/signature
                             annotations (via pdfjsLib.AnnotationMode.
                             ENABLE_FORMS), which a plain server-side text
                             dump cannot do. Do NOT route the lightweight
                             case through a plain text extraction - a
                             confirmed real regression: it drops position,
                             styling, and signature images entirely (see
                             ocr_router.run_lightweight_ocr's docstring for
                             the narrower cases that function is still
                             appropriate for)."""
        ...
        tmp_dir = tempfile.mkdtemp(prefix="ocr_analyze_")
        try:
    ...
    def _handle_ocr_process_router(self, body):
        ...
        file_name = _safe_filename(body.get("fileName"), "document.pdf")
        pdf_b64 = body.get("pdfBase64")
        force_strategy = body.get("forceStrategy")  # client already ran /api/ocr/analyze-strategy
        if not pdf_b64:
            raise ValueError("pdfBase64 is required")
        ...
        try:
            ...
            result = ocr_router.run_ocr(pdf_path, output_path, force_strategy=force_strategy)
    ...
    def _handle_aspose_test_translate(self, body):
        ...
        target_language = body.get("targetLanguage") or "English"
        mode = body.get("mode") or "structure"
        if not pdf_b64:
            raise ValueError("No PDF file provided.")

        import aspose_test_pipeline as asp_test

        tmp_dir = tempfile.mkdtemp(prefix="aspose_test_")
        try:
            pdf_path = os.path.join(tmp_dir, _safe_filename(file_name))
            with open(pdf_path, "wb") as f:
                f.write(base64.b64decode(pdf_b64))
            ...
            try:
                if mode == "full":
                    ...
                    t0 = time.time()
                    result = asp_test.run_structure_only_test(pdf_path, output_path)
                    log = [f"Converted via Aspose.Words Cloud (native PDF->DOCX, no translation) in {time.time()-t0:.1f}s"]
            ...
            return 200, {
                "ok": True,
                "outputBase64": output_b64,
                "outputFileName": "aspose-test-" + mode + "-" + os.path.splitext(file_name)[0] + ".docx",
                "log": log,
